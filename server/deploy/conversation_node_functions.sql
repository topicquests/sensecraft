...
CREATE POLICY conversation_node_select_policy ON public.conversation_node FOR SELECT USING (
  ...
  (status = 'role_draft' AND meta = 'channel' AND guild_id IS NOT NULL AND guild_id = public.is_playing_quest_in_guild(quest_id) AND public.has_guild_permission(guild_id, 'readAllRoleChannels')) OR
