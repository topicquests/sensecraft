...
import { guild1, guildCreator1, player1, player2, player3, player5 } from '../mocks/StoreMocks';
...
test.describe('Guild creator Permission Flow', () => {
  ...
  test('Guild creator can go to create guild page', async ({ page }) => {
    const row = page.locator('.guilds-table tbody tr', {
      has: page.locator('td', { hasText: guild1.name }),
    });
  ...
  test('Guild admin set guildCreator1 remove Researcher and add Game leader', async({page}) => {
    const row = page.locator('.guilds-table tbody tr', {
      has: page.locator('td', { hasText: guild1.name }),
    });
    const viewLink = row.locator('a', { hasText: 'Admin' });
  ...
  test('Guild admin set playerOne to Researcher, Philospher', async({page}) => {
    const row = page.locator('.guilds-table tbody tr', {
      has: page.locator('td', { hasText: guild1.name }),
    });
    const viewLink = row.locator('a', { hasText: 'Admin' });
  ...
  test('Guild admin set playerTwo remove Researcher and add Critic', async({page}) => {
    const row = page.locator('.guilds-table tbody tr', {
      has: page.locator('td', { hasText: guild1.name }),
    });
    const viewLink = row.locator('a', { hasText: 'Admin' });
  ...
  test('Guild admin set playerFour remove Researcher and add Scribe', async({page}) => {
    const row = page.locator('.guilds-table tbody tr', {
      has: page.locator('td', { hasText: guild1.name }),
    });
    const viewLink = row.locator('a', { hasText: 'Admin' });
    await viewLink.click();
    const qSelect = getRoleSelectByHandle(page, player5.handle!);
    await qSelect.click();
    await page.waitForSelector('.q-menu');
    const removeResearcher = page.locator('.q-menu .q-item', { hasText: 'Researcher' });
    await removeResearcher.click();
