...
import { mockQuest, questCreator } from '../mocks/StoreMocks';
...
test.describe('Quest creator Permission Flow', () => {
  ...
  test('Quest creator create quest', async ({ page }) => {
    ...
    await page.click('button[name="updateQuestBtn"]');
    await expect(
      page.getByRole('alert').filter({ hasText: 'Quest was updated successfully' })
    ).toBeVisible();
    await expect(page).toHaveURL(/\/quest\/\d+\/edit$/, { timeout: 10000 }); // waits up to 10 seconds
