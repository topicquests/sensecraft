...
import { guild1, guildCreator1, guildCreator1Conv } from '../utilities/StoreMocks';

test.describe('Guild 1 conversation', () => {
    test("GuildCreator1 creates and navigates to a new guild channel", async ({ page }) => {
      ...
      await expect(
        page.getByRole('alert').filter({ hasText: 'Added new conversation node' })
      ).toBeVisible();
      ...
      await page.getByRole('link', { name: guildCreator1Conv.title }).first().click();
