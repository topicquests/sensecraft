...
import { guild1, guildCreator1, player1, player2, player3, player4, player5 } from '../utilities/StoreMocks';
import { getRoleSelectByHandle, gotoGuildPage, signInPage } from '../utilities/utility';
...
test.describe('Guild admi  register to quest and set players roles', () => {
  ...
  test('Guild admin register to quest', async ({page}) =>{
    ...
    await expect (page.getByRole('alert').filter({hasText: 'You have registered to Quest'})).toBeVisible();
  })
